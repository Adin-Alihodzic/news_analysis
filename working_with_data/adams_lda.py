...
def sentiment_wordnet(content, top_words, word_probs):
...
all_words = {key: tf_feature_names for key in top_words.keys()}
comps = lda_model.components_
word_probs = {key: comps[i,:]/max(comps[i,:]) for i,key in enumerate(top_words.keys())}


sentiment_by_topic = {site: 0 for site in url_names}
for i,site in enumerate(url_names):
    print('Working on site: '+site)
    sentiment_by_topic[site] = sentiment_wordnet(df_no_nan.loc[df_no_nan['source'] == site]['processed_text'], all_words, word_probs)
...
for i in range(len(sentiment_by_topic.keys())):
    plt.subplot(4,5,i+1)
    score = []
    for j, topic in enumerate(top_words.keys()):
        score.append(sentiment_by_topic[url_names[i]][topic][3]*bias[j])
