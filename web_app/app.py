...
def make_mood_plot(tones, name):
    ...
    fig.savefig('static/img/mood_plots/predicted_article_'+name+'.png')
    ...
    if request.method == 'POST':
        ...
        if worked:
            ...
            return render_template('prediction_worked.html',
                                    ...
                                    script=script,
                                    div=div,
                                    mood_plot='src="static/img/mood_plots/predicted_article_'+str(name)+'.png"')
        else:
...
@app.route('/predict_graphs', methods=['POST','GET'])
def predict_graphs():
    if request.method == 'POST':
        sentence = str(request.form['sentence'])

        with open('../pickles/lda_modell_'+identifier+'.pkl', 'rb') as f:
            lda_model = pickle.load(f)
